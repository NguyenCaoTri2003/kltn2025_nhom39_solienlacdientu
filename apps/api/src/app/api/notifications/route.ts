...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Require Authorization header explicitly; do not fall back to cookies here
    if (!headerToken) {
  ...
  // userId is only supported in admin route
  ...
  // userId param is ignored for non-admin route
    ...
    const { items, total, totalPages, page: pg, pageSize: ps } = await notificationsUseCase.list({ userId: effectiveUserId, page, pageSize });
