...
/**
 * GET /api/students/[id]/failed-courses
 * - Returns failed courses for a student.
 * - Query: ?semesterId=123 (optional), ?compute=true (optional) to compute from raw grades if grade_summary/passed not available.
 */
export async function GET(
  ...
  const compute = String(searchParams.get("compute") || "").toLowerCase() === "true";

  const repo = new StudentRepository();
  const result = await repo.getStudentFailedCourses(studentId, semesterId, compute);
