...
export default function OfferingsList() {
  ...
  const summaryStats = useMemo(() => {
    ...
    const practice = filteredOfferings.filter((o) => o.is_practice_lecturer).length;
    const theory = total - practice;
    const totalCredits = filteredOfferings.reduce(
      (sum, o) => sum + (Number(o.courses?.credit) || 0),
      0
    );
    const totalCapacity = filteredOfferings.reduce(
      (sum, o) => sum + (Number(o.capacity) || 0),
      0
    );
    const totalRegistered = filteredOfferings.reduce(
      (sum, o) => sum + (Number(o.registered) || 0),
      0
    );

    const occupancy =
      totalCapacity > 0 ? Math.round((totalRegistered * 100) / totalCapacity) : 0;

    return { total, practice, theory, totalCredits, totalCapacity, totalRegistered, occupancy };
  }, [filteredOfferings]);
