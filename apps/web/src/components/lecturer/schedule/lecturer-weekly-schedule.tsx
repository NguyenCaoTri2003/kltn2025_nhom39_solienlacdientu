...
import { useLecturerSchedule } from "@/hooks/useLecturerSchedule";
import {
  CalendarDays,
  CalendarSearch,
  ChevronLeft,
...
export default function LecturerWeeklySchedule() {
  ...
  const colorMap: Record<string, string> = {
    practice:
      "border-emerald-500 dark:border-emerald-400 from-emerald-50/50 via-emerald-50/30 dark:from-emerald-800/30 dark:via-emerald-900/20",
    theory:
      "border-primary dark:border-blue-400 from-blue-50/50 via-blue-50/30 dark:from-blue-900/30 dark:via-blue-800/20",
    ...
    cancelled:
      "border-red-500 dark:border-red-400 from-red-50/60 via-red-50/40 dark:from-red-900/30 dark:via-red-800/20",
  };
    ...
    return (
      <div className="p-6">
        <EmptyState
          ...
          text="Chức năng này chỉ dành cho giảng viên."
        />
      </div>
    );
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-foreground">
                Lịch dạy, lịch coi thi theo tuần
              </h2>
              ...
              <table className="min-w-full border-collapse text-sm">
                <thead>
                  <tr className="bg-muted/30 border-b border-border/60">
                    <th className="p-3 text-left font-semibold text-foreground w-24">
                      Buổi
                    </th>
                              ...
                              <div className="flex flex-col gap-2">
                                {items.map((item) => (
                                  <Card
                                    key={item.id}
                                    className={cn(
                                      "relative rounded-xl border-l-4 p-3 transition-all hover:shadow-md bg-linear-to-br to-background/60 dark:to-background/80 opacity-100",
                                      ...
                                      <div className="space-y-1">
                                        <p className="text-[11px] text-muted-foreground dark:text-gray-400">
                                          <span className="font-medium">
                                            {item?.course_offering?.class?.name
                                              ? `${item.course_offering.class.name} - `
                                              : ""}
                                            {item?.course_offering?.class_code}
                                          </span>
                                        </p>
                                        <p className="text-[11px] text-muted-foreground dark:text-gray-400">
