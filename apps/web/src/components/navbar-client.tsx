...
export default function NavbarClient({ userRole, userName, avatarUrl, userId }: NavbarProps) {
  ...
  roleBasePath = userRole === 'admin' ? '/admin' : '/lecturer';
  const navItems = userRole === 'admin' ? adminNavItems : teacherNavItems;

  // ================= Admin Vertical Sidebar Layout =================
  if (userRole === 'admin') {
    return (
      <nav className="z-50">
        {/* Sidebar (desktop / md+) */}
            ...
            {navItems.map(item => {
              ...
              return (
                <button
                  ...
                  className={`group w-full flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium transition-all text-left ${
                    active
