...
export const WarningSummaryStats: React.FC<{ rows: Row[] }> = ({ rows }) => {
  ...
  return (
    <div className="mt-6 p-4 border rounded-lg bg-white dark:bg-card space-y-3">
      <h4 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Thống kê cảnh cáo học tập</h4>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div>
          <div className="font-semibold text-gray-700 dark:text-gray-300">Tổng số kết quả</div>
          <div className="text-2xl font-bold">{total}</div>
        </div>
        <div>
          <div className="font-semibold text-gray-700 dark:text-gray-300">Đã cảnh cáo</div>
          <div className="text-2xl font-bold">{warned}</div>
        </div>
        <div>
          <div className="font-semibold text-gray-700 dark:text-gray-300">Nguy cơ buộc thôi học</div>
          <div className="text-2xl font-bold text-red-600">{expulsion}</div>
        </div>
        <div>
          <div className="font-semibold text-gray-700 dark:text-gray-300">Cảnh cáo liên tiếp ≥2</div>
          <div className="text-2xl font-bold text-orange-600">{consecutive}</div>
        </div>
      </div>
      
      <div className="border-t pt-3 mt-3">
        <div className="font-semibold mb-2 text-gray-900 dark:text-gray-100">Tiêu chí vi phạm:</div>
