...
export function useSemesterSummaries(studentId: number | null, semesterIds: number[]) {
  ...
  useEffect(() => {
    // ❌ Không có student hoặc học kỳ thì reset luôn để không hiển thị nhầm
    ...
    async function loadSummaries() {
      ...
      try {
        ...
        console.error("Fetch semester summaries failed:", err);
        setSummaries({}); // ✅ Reset khi lỗi để tránh dữ liệu cũ
      } finally {
        setLoading(false);
      }
    }
    ...
    loadSummaries();
  }, [studentId, JSON.stringify(semesterIds)]); // ✅ so sánh chính xác mảng
